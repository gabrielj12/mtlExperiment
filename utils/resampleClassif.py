...
class resampleClassif:
    ...
    def __init__(self, task_id, clf, r_method, data_x, data_y, metric_list=["accuracy"], save_models=False, output_dir="output/models/"):
        self.__clf = clf
    ...
    def __loo(self):
        ...
        for train, test in kf.split(self.__data_x,y=self.__data_y):
            ...
            acc_train = getClassifMetrics(Y_train,predict_train,metrics_list=["accuracy"])
            predict_test = self.__clf.predict(X_test.drop(["id"],axis=1))
            acc_test = 1.0 if predict_test == Y_test.values.ravel() else 0.
            predicted_values = pd.concat([predicted_values, pd.concat([X_test["id"].reset_index(drop=True), pd.DataFrame(predict_test) ],axis=1)], axis = 0)
            print ("Fold {} ACC Train.{} ACC Test.{}".format(id_fold,acc_train,acc_test))
            if self.__save_models:
                with open(os.path.join(self.__output_dir,self.__task_id)+"/model_fold_{}.pickle".format(id_fold), 'wb') as handle:
            ...
            id_fold = id_fold + 1
        self.__predictions = predicted_values
        self.__final_metrics = getClassifMetrics(self.__data_y, self.__predictions)
